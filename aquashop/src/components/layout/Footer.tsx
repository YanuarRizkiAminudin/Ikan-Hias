...
const categoryLinks = [
  { label: 'Ikan Laut',      slug: 'ikan-laut' },
  { label: 'Ikan Air Tawar', slug: 'ikan-air-tawar' },
  { label: 'Ikan Koi',       slug: 'ikan-koi' },
  { label: 'Ikan Cupang',    slug: 'ikan-cupang' },
  { label: 'Ikan Discus',    slug: 'ikan-discus' },
  { label: 'Ikan Guppy',     slug: 'ikan-guppy' },
]
  ...
  return (
    <footer className="bg-text-dark text-white py-12">
      <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        {/* Kolom 2 — Kategori */}
        <div>
          <h3 className="font-bold text-sm uppercase tracking-wider text-gray-400 mb-4">Kategori</h3>
          <ul className="space-y-2" role="list">
            {categoryLinks.map(c => (
              <li key={c.slug}>
                <Link
                  to={`${ROUTES.CATALOG}?category=${c.slug}`}
        ...
        {/* Kolom 3 — Jam Buka */}
        <div>
          <h3 className="font-bold text-sm uppercase tracking-wider text-gray-400 mb-4">Jam Buka</h3>
          <p className="text-sm text-gray-400 leading-relaxed whitespace-pre-line">
            {storeInfo?.open_hours ?? 'Senin–Sabtu: 08.00–18.00\nMinggu: 09.00–15.00'}
          </p>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-10 pt-6 border-t border-gray-700 text-center text-xs text-gray-500">
        © {new Date().getFullYear()} {APP_NAME}. Semua harga adalah harga referensi, bukan harga checkout.
      </div>
    </footer>
  )
}
